...
def embedding(x, method='tsne', gamma=None, perplexity=30):
    if method == 'pca':
        trainer = PCA(n_components=2)
    elif method == 'kernel_pca':
        trainer = KernelPCA(n_components=2, kernel='rbf', gamma=None)
    elif method == 'lle':
        trainer = LocallyLinearEmbedding(n_components=2)
    else:
